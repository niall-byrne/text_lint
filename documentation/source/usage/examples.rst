...
- the schema file can be found `here <https://github.com/niall-byrne/text_lint/schemas/blob/main/schemas/makefile.yml>`__
- the Makefile itself can be found `here <https://github.com/niall-byrne/text_lint/blob/main/Makefile>`__
...
This `YAML <https://yaml.org/>`__ document describes the structure and contents of your target file(s) without being overly verbose.
...
Next a regex is defined that identifies lines that will be ignore (``comments``) in the target file during checks.

This can also set to ``null`` to disable this feature.
...
Each of these operations declares what **sequential** target file line (or lines) should consist of.

Together these assertions describe the structure of the file(s) as a whole.

Sequences
^^^^^^^^^

The ``assert_sequence_begins`` assertion allows us to apply a repeating series of assertions to the target file.

This can be an infinite (``count: -1``) or bounded set of repeating assertions.
...
Assertion operations also provide the ability to save extracted values via :ref:`regular expression <regular_expressions>` capture groups.

This allows additional processing in the :ref:`validators <validators>` stage.
...
Also make sure to follow the rule of ``double escaping``:
...
The ``validation operations`` consumes ``save id`` values that were created by the assertion operations.

These validation operations then combine and compare the values to ensure the content of the target file(s) is correct.
...
The validation operations themselves are built by combining ``lookup operations`` into ``lookup expressions``.

This enables the manipulation of any ``save id`` values that were collected by the ``assertion operations``.
...
There are a couple of lookup operations that are restricted in their usage.

These operations are not intended to be called multiple times within a lookup expression, and *must* be placed *before* any :ref:`transformation operation<transformation_lookup_operations>`.

Specifically these positional lookups are:

- ``capture``: which extracts a specific capture group from a save id. It should only be used once per lookup expression, as it makes little sense to perform operations on capture group and then switch to a different capture group.
...
.. _transformation_lookup_operations:

Transformation Lookup Operations
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Some lookup operations are meant for transforming data, and all start with the same indicator ‘``to``’.

Examples of this would be ``to_sorted`` or ``to_unique``.
...
Finally, you can also extract specific characters from words, or elements in arrays, by adding digits to your lookup expressions:
...
The ``validate_debug`` operation is incredibly useful when writing lookup expressions, as it provides a look at the underlying data being manipulated.
...
- a comment regular expression
- an assertion section
- a validator section

The :ref:`assertion section <assertions>` must contain one or more ``assertion operations`` that describe the structure of each text file.

The :ref:`validators section <validators>` *may* contain ``validation operations`` to examine the content of each text file with more scrutiny.

Combine ``lookup operations`` into :ref:`lookup expressions <lookup_expressions>` to create complex validation logic.
