...
class TestLoggingContexts:
  ...
  def test_main__calls_log_in_correct_sequence(
    ...
    mocked_linter.settings.file_path = "mocked/file/path"
    mocked_linter.settings.schema_path = "mocked/schema/path"

    with logging_contexts.main(mocked_linter):
      mocked_linter.log("inside context")

    assert mocked_linter.mock_calls == [
        mock.call.log(
        ...
        mock.call.log(
            ...
            ljust(LOGGING_COLUMN1_WIDTH) + mocked_linter.settings.file_path,
            indent=False,
            section=False,
        ),
        mock.call.log(
            logging_contexts.msg_log_filename_schema.
            ljust(LOGGING_COLUMN1_WIDTH) + mocked_linter.settings.schema_path,
            indent=False,
            section=False,
        ),
  ...
  def test_assertion__calls_log_in_correct_sequence(
    ...
    mocked_linter.textfile.index = 10

    with logging_contexts.assertion(mocked_linter, mocked_assertion):
      mocked_linter.log("inside context")
    ...
    assert mocked_linter.mock_calls == [
        ...
        mock.call.log(
            mocked_assertion,
            index=mocked_linter.textfile.index,
        ),
    ]
  ...
  def test_assertion_sequence__calls_log_in_correct_sequence(
    ...
    with logging_contexts.assertion_section(mocked_linter):
      mocked_linter.log("inside context")
  ...
  def test_validator__calls_log_in_correct_sequence(
    ...
    with logging_contexts.validator(mocked_linter, mocked_validator):
      mocked_linter.log("inside context")
  ...
  def test_validator_sequence__calls_log_in_correct_sequence(
    ...
    with logging_contexts.validator_section(mocked_linter):
      mocked_linter.log("inside context")
