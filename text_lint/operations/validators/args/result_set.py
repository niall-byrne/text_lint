...
class ResultSet:
  ...
  def validate_lookups(self) -> None:
    found_encoding_lookup = False
    for lookup in self.lookups:

      if lookup not in lookup_registry:
        continue

      if lookup.startswith(LOOKUP_TRANSFORMATION_PREFIX):
        found_encoding_lookup = True
      elif found_encoding_lookup:
        raise ValueError("Transformations belong at the end of the lookup.")
