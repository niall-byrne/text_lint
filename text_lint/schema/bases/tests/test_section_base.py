...
class TestSchemaSectionBase:
  ...
  def test_load__defined_section__returns_expected_instance(
    ...
    assert instances[0].operation == \
        schemas.one_simple_assertion[0]["operation"]
    mocked_operation_classes[instances[0].operation].assert_called_once_with(
        name=schemas.one_simple_assertion[0]["name"]
    )
  ...
  def test_load__defined_section__calls_overridable_hooks(
    ...
    cloned_schema = deepcopy(schemas.two_simple_assertions)
    ...
    assert mocked_create_operation_instance_hook.mock_calls == [
        mock.call(
            mocked_operation_classes[
                (schemas.two_simple_assertions[index]["operation"])],
            cloned_schema[index],
  ...
  def test_load__defined_section__invalid_operation__raises_schema_exception(
      ...
      concrete_schema_section_instance: SchemaSectionBase[mock.Mock],
      mocked_schema: mock.Mock,
      invalid_schema_assertions: schemas.AliasRawSchemaAssertions,
  ) -> None:
    with pytest.raises(SchemaError) as exc:
      concrete_schema_section_instance.load(deepcopy(invalid_schema_assertions))
    ...
    assert_is_schema_error(
        ...
        assertion_definition=invalid_schema_assertions[0],
        schema_path=mocked_schema.path,
    )
  ...
  @pytest.mark.parametrize("exception", [SplitGroupInvalid])
  def test_load__defined_section__split_group_invalid__raises_schema_exception(
      self,
      concrete_schema_section_instance: SchemaSectionBase[mock.Mock],
      mocked_operation_classes: Dict[str, Type[mock.Mock]],
      mocked_schema: mock.Mock,
