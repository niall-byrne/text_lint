...
translations-add:
	...
	@poetry run bash -c 'export GETTEXT_TRANSLATIONS_LANGUAGES; read -rp "Enter 2 character language code: " GETTEXT_TRANSLATIONS_LANGUAGES && poetry run pre-commit run --hook-stage=manual gettext-translations-add --all-files'
